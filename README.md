...
# ADT 
ptrn_adt - Abstract data type - абстрактный тип данных.

Шаблон позволяет спрятать реализацию, защитить данные от изменения в другом модуле (чтобы модуль ничего не знал о структуре данных). 

Цель изолировать изменения в одном единственном модуле.
Применяется для разделения интерфейса от применения, таким образом достигается скрытие информации.
Применяется для объектов, экземпляров которых может быть много.
Основывается на способности С реализовывать незаконченные типы (incomplete types).
...
state_pattern.c, state_pattern.h - шаблон состояния.
...
#### Тест Semaforo
ptrn_test_gmock - тест машины состояния светофора.
