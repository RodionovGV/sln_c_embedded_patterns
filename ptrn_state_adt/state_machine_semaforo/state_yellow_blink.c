...
static void yellow_blink_state(StateSemaforoPtr s, StateSemEvents events){

    // ON_ENTRY
    if (s_data.first_entry) {
        LOG_DBG_TEXT("Yellow Blink state ENTER");
...
exit_from_state:
    LOG_DBG_TEXT("Yellow Blink state EXIT");
    init_state(&s_data);
    if (events.on_exit) {
...
void transitionToYellowBlink(StateSemaforoPtr s) {
    LOG_DBG_TEXT("[TRANSITION to Yellow Blink]");
    defaultSemaforoImplementation(s);
    s->name = "yellow_blink_state";
    s->state_yellow_blink = yellow_blink_state;
}
