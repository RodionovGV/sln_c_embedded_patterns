...
static void green_state(StateSemaforoPtr s, StateSemEvents events){

    // ON_ENTRY
    if (s_data.first_entry) {
        LOG_DBG_TEXT("Green state ENTER");
    ...
    if (events.check_change_state) {
        ...
        if (delta > events.workTimeToSwitch) {
            ...
            //LOG_DBG_TEXT("NEED TO SWITCH Red Yellow");
            //transitionToRedYellow(s);
            LOG_DBG_TEXT("NEED TO SWITCH Green Blink");
            transitionToGreenBlink(s);
...
void transitionToGreen(StateSemaforoPtr s) {
    LOG_DBG_TEXT("[TRANSITION to Green]");
    defaultSemaforoImplementation(s);
    s->name = "green_state";
    s->state_green= green_state;
}
