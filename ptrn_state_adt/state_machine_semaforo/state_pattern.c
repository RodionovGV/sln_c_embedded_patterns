...
static void pattern_state(StateSemaforoPtr s, StateSemEvents events){

    // ON_ENTRY
    if (s_data.first_entry) {
        LOG_DBG_TEXT("Pattern state ENTER");
    ...
    if (events.check_change_state) {
        ...
        if (delta > events.workTimeToSwitch) {
            LOG_DBG_TEXT("NEED TO SWITCH XXX");
            transitionToRed(s);
...
exit_from_state:
    LOG_DBG_TEXT("Pattern state EXIT");
    init_state(&s_data);
    if (events.on_exit) {
...
void transitionToPattern(StateSemaforoPtr s) {
    LOG_DBG_TEXT("[TRANSITION to Pattern]");
    defaultSemaforoImplementation(s);
    //s->name = "pattern_state";
    //s->state_pattern = pattern_state;
}
