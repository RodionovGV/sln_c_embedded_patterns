...
static void green_blink_state(StateSemaforoPtr s, StateSemEvents events){

    // ON_ENTRY
    if (s_data.first_entry) {
        LOG_DBG_TEXT("Green Blink state ENTER");
...
exit_from_state:
    LOG_DBG_TEXT("Green Blink state EXIT");
    init_state(&s_data);
    if (events.on_exit) {
...
void transitionToGreenBlink(StateSemaforoPtr s) {
    LOG_DBG_TEXT("[TRANSITION to Green Blink]");
    defaultSemaforoImplementation(s);
    s->name = "green_blink_state";
    s->state_green_blink= green_blink_state;
}
