// Клиентский код. Код использует предоставленный интерфейс.
...
// Фукция запускающая функцию состояния.
void tickWatch(DigitalStopWatchPtr instance)
{
	instance->state.tick(&instance->state);
}

// Фукция запускающая функцию состояния.
void runState(DigitalStopWatchPtr instance)
{
	instance->state.fxn(&instance->state);
}